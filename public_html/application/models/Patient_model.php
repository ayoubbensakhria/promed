...
class Patient_model extends CI_Model {
    var $column_order = array('patients.patient_name','patients.patient_unique_id','patients.guardian_name','patients.gender','patients.mobileno','staff.name','opd_details.appointment_date','opd_details.patient_id'); //set column field database for datatable orderable
    var $column_search = array('patient_name','patient_unique_id','guardian_name','mobileno');
    var $columncredential_order = array('patient_name','patient_unique_id','username','password'); //set column field database for datatable orderable
    var $columncredential_search = array('patient_name','patient_unique_id','username','password');
    var $columnipd_order = array('patients.patient_name','ipd_details.ipd_no','patients.patient_unique_id','patients.gender','patients.mobileno','staff.name','bed.name','charges','payment','amountdue','ipd_details.credit_limit'); //set column field database for datatable orderable
    var $columnipd_search = array('patients.patient_name','ipd_details.ipd_no','patients.patient_unique_id','patients.gender','patients.mobileno','staff.name','bed.name','ipd_details.credit_limit');
    var $columnpatient_order = array('patient_unique_id','patient_name','age','gender','mobileno','guardian_name','address','action'); //set column field database for datatable orderable
    var $columnpatient_search = array('patient_unique_id','patient_name','age','gender','mobileno','guardian_name','address');  
 ...
 // ===================== search datatable for Patient ==================================
    ...
    // ===================== search datatable for OPD ==================================
     ...
     public function search_datatablecredential() {       
        ...
        $this->db->limit($_POST['length'],$_POST['start']);
        $this->db->order_by($this->columncredential_order[$_POST['order'][0]['column']],$_POST['order'][0]['dir']);         $query = $this->db->get();
        $result = $query->result();
        return $result;
        }
